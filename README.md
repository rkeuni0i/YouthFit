...
│     링크 고도화   │ • 서류 발급: 정부24 메인이 아닌 해당 민원 고유 고유코드(CappBizCD) 및  │
...
- 4대 KPI 지표 카드, 3개 인터랙티브 탭, 실시간 검색, 5초 자동 갱신 지원.
...
- **누적 회원 및 활성 청년**: 등록된 계정 수 및 최근 24시간 이내 서비스 활동 인원.
...
### 2. 회원 인증 및 보관함 API
- `POST /api/auth/register`: 이메일/비밀번호 신규 회원가입 (PBKDF2 해싱)
- `POST /api/auth/login`: 회원 로그인 및 사용자 세션 반환
- `POST /api/auth/google`: Google 소셜 계정 간편 연동
- `POST /api/auth/save-diagnosis`: 현재 진단 결과 및 추천 패키지 유저 보관함 저장
- `GET /api/auth/me`: 현재 로그인 사용자의 정보 및 과거 진단 이력 조회

### 3. 사용자 인터랙션 & 텔레메트리 API
- `POST /api/stats/policy-click`: 모달 열람, 외부 공식 신청처 이동, 요약본 클릭 이벤트 추적

### 4. 통합 관리자 포털 (Admin Portal) API
...
별도의 외부 데이터베이스 설치 없이도 내장된 SQLite로 즉시 100% 가동됩니다.
...
```
> [!NOTE]
> `data/youthfit.db`에 이미 2026년 유효 일정의 핵심 12종 정책과 423건의 온통청년 정책(총 435건)이 완벽히 적재되어 있어 바로 서버를 구동하셔도 무방합니다.
...
YouthFit AI는 핵심 기능 및 최근 오류 수정 사항에 대한 자동화 검증 스크립트를 갖추고 있습니다:

```bash
# 전체 오류 수정 및 신규 기능 통합 검증 (7대 테스트 케이스)
python scripts/test_youthfit_fixes.py
```

### 테스트 통과 항목 (7/7 Pass)
- [x] **대시보드 UI 무결성**: 사이드바 하단 적합도 점수 카드 가림 없음 검증 (`dashboard.html`)
- [x] **관리자 포털 UI 가동**: 4대 KPI 및 3대 탭(인기 정책, 유저 로그, 서비스 텔레메트리) 렌더링 검증 (`admin.html`)
...
- [x] **관리자 개요 API**: KPI 집계 데이터 정상 반환 검증 (`/api/admin/overview`)
