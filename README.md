...
4. [주요 서비스 화면 & 사용자 플로우](#-주요-서비스-화면--사용자-플로우)
   ...
   - [5. 유저 프로필 및 맞춤 조건 관리 (`profile.html`)](#5-유저-프로필-및-맞춤-조건-관리-profilehtml)
   - [6. 전체 정책 탐색기 (`explorer.html`)](#6-전체-정책-탐색기-explorerhtml)
   - [7. 통합 관리자 모니터링 포털 (`admin.html`)](#7-통합-관리자-모니터링-포털-adminhtml)
...
├───────────────────┼─────────────────────────────────────────────────────────────────────────┤
│ [9] Google OAuth  │ • renderPersonalizedHero 내 구문 에러(try 누락) 수정으로 렌더링 중단 차단│
│     참조 에러 해소 │ • clearAuthUser 미정의 상황 방어 로직 및 logoutUser 자동 폴백 연동 완료 │
├───────────────────┼─────────────────────────────────────────────────────────────────────────┤
│ [10] 유저 프로필   │ • 상단 네비게이션 헤더의 로그인 계정 배지('OOO 님') 클릭 시 profile.html│
│     관리 & DB 동기│   직결 및 프로필 아이콘 링크 지원                                       │
│     (profile.html)│ • 이름, 비밀번호(일반 가입), 5대 진단 조건 실시간 수정 및 DB 즉시 저장  │
│                   │ • POST /api/user/update-account 신규 구축으로 users 테이블 영구 동기화  │
├───────────────────┼─────────────────────────────────────────────────────────────────────────┤
│ [11] 서류 리스트   │ • '⚠️중요⚠️', '공고일 이후 발급분' 등 무관한 공지사항 텍스트 완벽 필터링 │
│     공지 정제 &    │ • 체크리스트 분모 하드코딩(/ 3건) 제거 및 동적 총 건수(/ N건) 연동     │
│     동적 카운터   │ • 대시보드 첫 진입 시 서류가 자동 체크되던 버그 수정(기본 0건/0% 초기화)│
└───────────────────┴─────────────────────────────────────────────────────────────────────────┘
```
...
flowchart TD
    ...
    D --> P["유저 프로필 & 조건 관리<br>(profile.html)"]
...
- **인터랙티브 서류 체크리스트 고도화**:
  - **공지사항 문구 자동 필터링**: 원문 공고문의 `⚠️중요⚠️`, `공고일 이후 발급분만 인정`, `온라인 제출)` 등 서류와 무관한 안내 텍스트를 백엔드·프론트엔드 이중 필터링으로 완벽 제거하고 순수 증빙 서류만 노출.
  - **동적 진행률 카운터**: 기존 `/ 3건 완료` 하드코딩 분모 결함을 실제 추출된 서류 건수(`<span id="progress-total">N</span>건 완료`)로 동적 반영.
  - **기본 미선택 초기화**: 로딩 콘솔에서 대시보드로 첫 진입 시 첫 번째 서류가 자동 체크되던 현상을 해결하여 기본 `0 / N건 완료 (0%, 미발급)` 상태로 정밀 초기화.
  - **1:1 정부24 직결**: 서류별 정부24 고유 서비스 코드(`CappBizCD`) 및 공공기관 1:1 직결 발급 링크 제공.
...
### 5. 유저 프로필 및 맞춤 조건 관리 (`profile.html`)
- **헤더 계정 배지 원클릭 진입**: 상단 네비게이션 헤더의 로그인 사용자 뱃지(`{user.name} 님`) 및 프로필 아이콘 클릭 시 `/profile.html`로 즉시 연결.
- **회원 정보 및 비밀번호 안전 변경**: 이름(닉네임) 변경 및 기존 비밀번호 PBKDF2 암호화 검증 기반 신규 비밀번호 변경 (일반 가입 계정 지원, Google 간편가입 계정은 안내 라벨 노출).
- **5대 핵심 진단 조건 실시간 수정**: 나이(슬라이더), 거주지역(전국 17개 시도 및 서울 25개 자치구), 취업상태, 가구형태, 기준 중위소득 분위 직관적 변경.
- **데이터베이스 실시간 영구 동기화**: `POST /api/user/update-account` 엔드포인트를 통해 `users` DB 테이블에 변경 사항 영구 저장 및 활성 세션 자동 반영.
- **원클릭 맞춤 재진단 연동**: 변경된 프로필 조건을 바탕으로 즉시 새로운 맞춤 진단을 실행하여 업데이트된 대시보드로 이동하는 퀵 액션 지원.

### 6. 전체 정책 탐색기 (`explorer.html`)
- 적재된 **전체 435건(온통청년 423건 + 서울시/전국 핵심 12건)** 정책 실시간 검색 및 필터링.
- 대분류(일자리, 주거, 금융·복지·문화, 교육·직업훈련), 지역, 연령별 실시간 다이나믹 필터링.

### 7. 통합 관리자 모니터링 포털 (`admin.html`)
...
│   ├── profile.html               # [NEW] 회원 프로필 정보(이름, 비밀번호) 및 5대 맞춤 문진 조건 관리
...
- `POST /api/user/update-account`: [NEW] 사용자 이름(닉네임), 비밀번호(PBKDF2 검증) 및 5대 진단 조건 DB 통합 동기화
- `GET /api/user/profile`: [NEW] 로그인 사용자의 최신 계정 프로필 및 5대 맞춤 문진 조건 상세 조회
...
- 👤 **회원 프로필 및 조건 관리**: [`http://127.0.0.1:8000/profile.html`](http://127.0.0.1:8000/profile.html)
...
YouthFit AI는 시스템 안정성과 보안을 위해 11대 통합 자동화 검증 체계를 갖추고 있습니다:

```bash
# 전체 오류 수정, RBAC 및 DB 연동 통합 검증
python scripts/test_youthfit_fixes.py
```

### 테스트 통과 항목 (11/11 Pass 100%)
...
- [x] **서류 체크리스트 정제 및 동적 카운터**: 공지 문구(`⚠️중요⚠️` 등) 자동 제외 및 실제 서류 건수 기반 동적 분모(`progress-total`), 기본 미발급(0건 완료, 0%) 초기화 검증
- [x] **유저 프로필 & DB 동기화 API**: 사용자 이름 변경, PBKDF2 암호화 비밀번호 변경, 5대 문진 조건 `users` DB 실시간 동기화 검증 (`/api/user/update-account`)
